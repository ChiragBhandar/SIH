...
const ORGANIZATIONS = [
  { id: "org-1", name: "Highland Apiaries Cooperative", type: "Producer" },
  { id: "org-2", name: "Apex Honey Processors Ltd", type: "Processor" },
  { id: "org-3", name: "BioTrace Analytical Labs", type: "Laboratory" },
];
  ...
  return (
    <header className="sticky top-0 z-40 flex h-14 w-full items-center justify-between border-b border-border bg-background/95 px-4 backdrop-blur-xs sm:px-6">
      ...
      <div className="flex items-center gap-2 sm:gap-3">
        ...
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            <Button
              variant="outline"
              size="sm"
              className="h-8 gap-1.5 px-2.5 text-xs font-normal max-w-[190px] truncate sm:max-w-none"
            >
          ...
          <DropdownMenuContent align="end" className="w-64">
            <DropdownMenuLabel className="text-xs">Active Organisation</DropdownMenuLabel>
            <DropdownMenuSeparator />
            {ORGANIZATIONS.map((org) => {
              const isSelected = org.name === currentOrg;
              return (
                <DropdownMenuItem
                  key={org.id}
                ...
                >
                  <div className="flex flex-col">
                    ...
                    <span className="text-[11px] text-muted-foreground">{org.type}</span>
                  </div>
        ...
        {/* User profile dropdown with Role context demo */}
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            <button
              type="button"
          ...
          <DropdownMenuContent align="end" className="w-56">
            <DropdownMenuLabel className="flex flex-col space-y-0.5">
              <span className="text-xs font-semibold text-foreground">Chirag Operator</span>
              <span className="text-[11px] text-muted-foreground">operator@honeychain.io</span>
            </DropdownMenuLabel>
            <DropdownMenuSeparator />
            <DropdownMenuLabel className="text-[11px] font-normal text-muted-foreground">
              Simulate Role Context:
            </DropdownMenuLabel>
            {(["super_admin", "org_admin", "beekeeper", "processor", "lab_technician"] as UserRole[]).map(
              (role) => (
                <DropdownMenuItem
                  key={role}
            ...
            <DropdownMenuItem className="text-xs text-rose-600 focus:text-rose-600 cursor-pointer">
